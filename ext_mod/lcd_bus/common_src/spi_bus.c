...
/* end includes */
    ...
    */
    ...
    static mp_obj_t mp_lcd_spi_bus_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *all_args)
    {
        LCD_DEBUG_PRINT("Entering mp_lcd_spi_bus_make_new\n")
        enum {
            ARG_spi_bus,
            ARG_dc,
            ARG_freq,
        ...
        self->spi_bus = (mp_machine_hw_spi_device_obj_t *)MP_OBJ_TO_PTR(args[ARG_spi_bus].u_obj);
        ...
        self->panel_io_config.dc_gpio = (mp_hal_pin_obj_t) args[ARG_dc].u_int;
        ...
        LCD_DEBUG_PRINT("sck_gpio_num=%d\n", mp_obj_get_int(self->spi_bus->spi_bus->sck))
        LCD_DEBUG_PRINT("mosi_gpio_num=%d\n", mp_obj_get_int(self->spi_bus->spi_bus->mosi))
        LCD_DEBUG_PRINT("miso_gpio_num=%d\n", mp_obj_get_int(self->spi_bus->spi_bus->miso))
        LCD_DEBUG_PRINT("dc_gpio_num=%d\n", self->panel_io_config.dc_gpio)
        LCD_DEBUG_PRINT("cs_gpio_num=%d\n", mp_obj_get_int(self->panel_io_config.cs_gpio))
        //LCD_DEBUG_PRINT("spi_mode=%d\n", self->panel_io_config.spi_mode)
        LCD_DEBUG_PRINT("baudrate=%d\n", self->freq)
        ...
        LCD_DEBUG_PRINT("Leaving mp_lcd_spi_bus_make_new\n")
    ...
    mp_lcd_err_t SPI_init(mp_obj_t obj, uint16_t width, uint16_t height, uint8_t bpp, uint32_t buffer_size, bool rgb565_byte_swap, uint8_t cmd_bits, uint8_t param_bits)
    {
        LCD_DEBUG_PRINT("Entering SPI_init\n")
        ...
        LCD_DEBUG_PRINT("arguments defined\n")
        */
        ...
        LCD_DEBUG_PRINT("Leaving SPI_init\n")
        return LCD_OK;
    }
    ...
    mp_lcd_err_t SPI_get_lane_count(mp_obj_t obj, uint8_t *lane_count)
    {
        LCD_DEBUG_PRINT("Entering SPI_get_lane_count\n")
        LCD_UNUSED(obj);
        *lane_count = 1;
        LCD_DEBUG_PRINT("Entering SPI_get_lane_count\n")
        return LCD_OK;
    ...
    mp_lcd_err_t SPI_rx_param(mp_obj_t obj, int lcd_cmd, void *param, size_t param_size)
    {
        LCD_DEBUG_PRINT("Entering SPI_rx_param \n")
    ...
    mp_lcd_err_t SPI_tx_param(mp_obj_t obj, int lcd_cmd, void *param, size_t param_size)
    {
        LCD_DEBUG_PRINT("SPI_tx_param: cmd=%x, # of param=%d\n", lcd_cmd, (int) param_size)
    ...
    mp_lcd_err_t SPI_tx_color(mp_obj_t obj, int lcd_cmd, void *color, size_t color_size, int x_start, int y_start, int x_end, int y_end, uint8_t rotation, bool last_update)
    {
        LCD_DEBUG_PRINT("Entering SPI_tx_color\n")
        LCD_UNUSED(x_start);
        LCD_UNUSED(y_start);
        LCD_UNUSED(x_end);
        LCD_UNUSED(y_end);
    ...
    mp_obj_t SPI_bus_get_host(mp_obj_t obj)
    {
        LCD_DEBUG_PRINT("Entering SPI_bus_get_host\n")
        mp_lcd_spi_bus_obj_t *self = (mp_lcd_spi_bus_obj_t *)obj;
        LCD_DEBUG_PRINT("Leaving SPI_bus_get_host\n")
        return mp_obj_new_int(self->host);
    }

    MP_DEFINE_CONST_FUN_OBJ_1(SPI_bus_get_host_obj, SPI_bus_get_host);
    ...
    */
    ...
    {
        self->panel_io_config.spi_transfer(
            MP_OBJ_FROM_PTR(self->spi_bus), param_size * 2, (const uint8_t *) param, NULL
        );
    }
    ...
    {
        self->panel_io_config.spi_transfer(
            MP_OBJ_FROM_PTR(self->spi_bus), param_size, (const uint8_t *) param, NULL
        );
    }
    ...
    void send_cmd_16(mp_lcd_spi_bus_obj_t *self, int lcd_cmd)
    {
        ...
        if (lcd_cmd >= 0x00) {
            self->panel_io_config.spi_transfer(
                MP_OBJ_FROM_PTR(self->spi_bus), 2,
    ...
    void send_cmd_8(mp_lcd_spi_bus_obj_t *self, int lcd_cmd)
    {
        ...
        if (lcd_cmd >= 0x00) {
            self->panel_io_config.spi_transfer(
                MP_OBJ_FROM_PTR(self->spi_bus), 1,
...
#ifndef MP_PORT_UNIX
    { MP_ROM_QSTR(MP_QSTR_get_host),             MP_ROM_PTR(&SPI_bus_get_host_obj)             },
#endif
